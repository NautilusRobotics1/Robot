...
cv::Mat static processFrame (cv::Mat &imgSrc, std::vector<std::vector<cv::Point> > &_ptContours, std::vector<cv::Vec4i> &_v4iHierarchy) {
	...
	cv::inRange(imgSrc, cv::Scalar(low_b, low_g, low_r), cv::Scalar(high_b, high_g, high_r), imgThresh);
	cv::imshow("imgThresh", imgThresh);
	// convert to grayscale
	//cv::cvtColor(imgThresh, imgGrayscale, CV_BGR2GRAY); // Isn't really grayscale though
